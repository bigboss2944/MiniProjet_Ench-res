...
<h3>Mon profil</h3>
	<form action="ModifierProfilServlet" method="post">
		...
		Téléphone :<input type="text" name="telephone" pattern ="[0-9]*" placeholder="${user.telephone}"/><BR />
		Rue :<input type="text" name="rue"  placeholder="${user.rue}" /><BR />
		Code postal :<input type="text" name="codePostal"  pattern ="[0-9]*" placeholder="${user.codePostal}"/><BR />
		Ville :<input type="text" name="ville" placeholder="${user.ville}" /><BR />
		Mot de passe actuel :<input type="password" name="motDePasse"  valeur="" required /><BR />
